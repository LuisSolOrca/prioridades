...
export async function POST(
  ...
  try {
    ...
    const { action, name, importance, currentState, notes } = body;

    const message = await ChannelMessage.findById(params.messageId);
    if (!message) return NextResponse.json({ error: 'No encontrado' }, { status: 404 });
    if (message.commandType !== 'values-wheel') return NextResponse.json({ error: 'Tipo incorrecto' }, { status: 400 });
    if (message.commandData.closed) return NextResponse.json({ error: 'Cerrado' }, { status: 400 });

    const userId = (session.user as any).id;
    const userName = (session.user as any).name || 'Usuario';

    if (!message.commandData.values) message.commandData.values = [];

    if (action === 'add') {
      message.commandData.values.push({
...
export async function PATCH(
  ...
  try {
    ...
    const userId = (session.user as any).id;
    const idx = message.commandData.values?.findIndex((v: any) => v.id === valueId);
    if (idx === -1 || idx === undefined) return NextResponse.json({ error: 'No encontrado' }, { status: 400 });

    if (message.commandData.values[idx].userId !== userId && (session.user as any).role !== 'ADMIN') {
      return NextResponse.json({ error: 'No autorizado' }, { status: 403 });
    }

    message.commandData.values.splice(idx, 1);
    message.markModified('commandData');
    await message.save();
...
export async function DELETE(
  ...
  try {
    ...
    notifyDynamicClosed({
      ...
      commandType: 'values-wheel', commandData: message.commandData,

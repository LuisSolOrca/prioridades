...
export async function POST(
  ...
  try {
    ...
    const { action, section, text } = body;

    const message = await ChannelMessage.findById(params.messageId);
    if (!message) return NextResponse.json({ error: 'No encontrado' }, { status: 404 });
    if (message.commandType !== 'metaphor-canvas') return NextResponse.json({ error: 'Tipo incorrecto' }, { status: 400 });
...
export async function DELETE(
  ...
  try {
    ...
    notifyDynamicClosed({
      ...
      commandType: 'metaphor-canvas', commandData: message.commandData,

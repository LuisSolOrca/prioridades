...
/**
 * POST /api/projects/[id]/messages/[messageId]/innovation-matrix
 * Add item
 */
export async function POST(
  ...
  try {
    ...
    const { quadrant, text } = await request.json();

    const validQuadrants = ['incremental', 'adjacent', 'disruptive', 'radical'];
    if (!validQuadrants.includes(quadrant) || !text?.trim()) {
    ...
    if (message.commandType !== 'innovation-matrix') {
      return NextResponse.json({ error: 'No es una Innovation Matrix' }, { status: 400 });
    }
    ...
    console.error('Error in innovation-matrix add:', error);
    return NextResponse.json({ error: 'Error al agregar' }, { status: 500 });
  }
}

/**
 * PATCH /api/projects/[id]/messages/[messageId]/innovation-matrix
 * Delete item
 */
export async function PATCH(
  ...
  try {
    ...
    console.error('Error in innovation-matrix delete:', error);
    return NextResponse.json({ error: 'Error al eliminar' }, { status: 500 });
  }
}

/**
 * DELETE /api/projects/[id]/messages/[messageId]/innovation-matrix
 * Close (only creator)
 */
export async function DELETE(
  ...
  try {
    ...
    console.error('Error closing innovation-matrix:', error);

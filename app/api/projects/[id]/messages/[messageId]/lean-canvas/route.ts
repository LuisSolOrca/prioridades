...
const BLOCK_KEYS = [
  'problem', 'customerSegments', 'uniqueValue', 'solution', 'channels',
  'revenueStreams', 'costStructure', 'keyMetrics', 'unfairAdvantage'
];

/**
 * POST /api/projects/[id]/messages/[messageId]/lean-canvas
...
export async function POST(
  ...
  try {
    ...
    if (!blockId || !text?.trim() || !BLOCK_KEYS.includes(blockId)) {
    ...
    if (message.commandType !== 'lean-canvas') {
      return NextResponse.json({ error: 'No es un Lean Canvas' }, { status: 400 });
    }
    ...
    console.error('Error in lean-canvas add:', error);
    return NextResponse.json({ error: 'Error al agregar' }, { status: 500 });
  }
}

/**
 * PATCH /api/projects/[id]/messages/[messageId]/lean-canvas
...
export async function PATCH(
  ...
  try {
    ...
    console.error('Error in lean-canvas delete:', error);
    return NextResponse.json({ error: 'Error al eliminar' }, { status: 500 });
  }
}

/**
 * DELETE /api/projects/[id]/messages/[messageId]/lean-canvas
...
export async function DELETE(
  ...
  try {
    ...
    console.error('Error closing lean-canvas:', error);

...
export async function POST(
  ...
  try {
    ...
    const { action, type, original, challenge, clarified } = body;

    const message = await ChannelMessage.findById(params.messageId);
    if (!message) return NextResponse.json({ error: 'No encontrado' }, { status: 404 });
    if (message.commandType !== 'metamodel-board') return NextResponse.json({ error: 'Tipo incorrecto' }, { status: 400 });
    if (message.commandData.closed) return NextResponse.json({ error: 'Cerrado' }, { status: 400 });

    const userId = (session.user as any).id;
    const userName = (session.user as any).name || 'Usuario';

    if (!message.commandData.patterns) message.commandData.patterns = [];

    if (action === 'add') {
      message.commandData.patterns.push({
...
export async function PATCH(
  ...
  try {
    ...
    const userId = (session.user as any).id;
    const idx = message.commandData.patterns?.findIndex((p: any) => p.id === patternId);
    if (idx === -1 || idx === undefined) return NextResponse.json({ error: 'No encontrado' }, { status: 400 });

    if (message.commandData.patterns[idx].userId !== userId && (session.user as any).role !== 'ADMIN') {
      return NextResponse.json({ error: 'No autorizado' }, { status: 403 });
    }

    message.commandData.patterns.splice(idx, 1);
    message.markModified('commandData');
    await message.save();
...
export async function DELETE(
  ...
  try {
    ...
    notifyDynamicClosed({
      ...
      commandType: 'metamodel-board', commandData: message.commandData,

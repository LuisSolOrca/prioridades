...
export async function POST(
  ...
  try {
    ...
    const { action, state, physiology, focus, language } = body;

    const message = await ChannelMessage.findById(params.messageId);
    if (!message) return NextResponse.json({ error: 'No encontrado' }, { status: 404 });
    if (message.commandType !== 'action-triads') return NextResponse.json({ error: 'Tipo incorrecto' }, { status: 400 });
    if (message.commandData.closed) return NextResponse.json({ error: 'Cerrado' }, { status: 400 });

    const userId = (session.user as any).id;
    const userName = (session.user as any).name || 'Usuario';

    if (!message.commandData.triads) message.commandData.triads = [];

    if (action === 'add') {
      message.commandData.triads.push({
...
export async function PATCH(
  ...
  try {
    ...
    await connectDB();
    const { triadId } = await request.json();
    ...
    const userId = (session.user as any).id;
    const idx = message.commandData.triads?.findIndex((t: any) => t.id === triadId);
    if (idx === -1 || idx === undefined) return NextResponse.json({ error: 'No encontrado' }, { status: 400 });

    if (message.commandData.triads[idx].userId !== userId && (session.user as any).role !== 'ADMIN') {
      return NextResponse.json({ error: 'No autorizado' }, { status: 403 });
    }

    message.commandData.triads.splice(idx, 1);
    message.markModified('commandData');
    await message.save();
...
export async function DELETE(
  ...
  try {
    ...
    notifyDynamicClosed({
      ...
      commandType: 'action-triads', commandData: message.commandData,

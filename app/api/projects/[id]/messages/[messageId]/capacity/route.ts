...
export async function DELETE(req: Request, { params }: { params: { id: string; messageId: string } }) {
  try {
    ...
    message.markModified('commandData');
    await message.save();
    notifyDynamicClosed({ projectId: params.id, channelId: message.channelId, messageId: params.messageId, commandType: 'capacity', commandData: message.commandData, closedByUserId: (session.user as any).id, closedByUserName: (session.user as any).name || 'Usuario' }).catch(err => console.error('Error notifying dynamic closed:', err));
    const populated = await populate(ChannelMessage.findById(message._id));
    try { await triggerPusherEvent(`presence-channel-${message.channelId}`, 'message-updated', populated); } catch (e) { console.error(e); }
    return NextResponse.json(populated);
  } catch (error) {

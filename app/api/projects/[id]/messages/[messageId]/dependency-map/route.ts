...
export async function POST(req: Request, { params }: { params: { id: string; messageId: string } }) {
  try {
    ...
    await connectDB();
    const { name, dependencies } = await req.json();
    if (!name?.trim()) return NextResponse.json({ error: 'Faltan datos' }, { status: 400 });
    const message = await ChannelMessage.findById(params.messageId);
    if (!message || message.commandType !== 'dependency-map' || message.commandData.closed) return NextResponse.json({ error: 'Error' }, { status: 400 });
    message.commandData.tasks.push({ name: name.trim(), dependencies: dependencies || [], status: 'pending' });
...
export async function PATCH(req: Request, { params }: { params: { id: string; messageId: string } }) {
  try {
    ...
    const message = await ChannelMessage.findById(params.messageId);
    if (!message || message.commandType !== 'dependency-map' || message.commandData.closed) return NextResponse.json({ error: 'Error' }, { status: 400 });
    const task = message.commandData.tasks[taskIndex];
...
export async function DELETE(req: Request, { params }: { params: { id: string; messageId: string } }) {
  try {
    ...
    await connectDB();
    const message = await ChannelMessage.findById(params.messageId);
    if (!message || message.commandType !== 'dependency-map') return NextResponse.json({ error: 'Error' }, { status: 404 });
    ...
    message.markModified('commandData');
    await message.save();
    notifyDynamicClosed({ projectId: params.id, channelId: message.channelId, messageId: params.messageId, commandType: 'dependency-map', commandData: message.commandData, closedByUserId: (session.user as any).id, closedByUserName: (session.user as any).name || 'Usuario' }).catch(err => console.error('Error notifying dynamic closed:', err));
    const populated = await populate(ChannelMessage.findById(message._id));
    try { await triggerPusherEvent(`presence-channel-${message.channelId}`, 'message-updated', populated); } catch (e) { console.error(e); }
    return NextResponse.json(populated);
  } catch (error) {

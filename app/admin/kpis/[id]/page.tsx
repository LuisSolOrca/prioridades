...
import IframeFormulaEditor from '@/components/kpis/IframeFormulaEditor';
...
export default function EditKPIPage() {
  ...
  }, [status, session, router, kpiId]);

  const loadData = async () => {
    try {
      const [kpiRes, initiativesRes, usersRes] = await Promise.all([
      ...
      // Cargar datos del KPI en el formulario
      setName(kpiData.name);
      setDescription(kpiData.description || '');
      setStrategicObjective(kpiData.strategicObjective);
      setInitiativeId(kpiData.initiativeId._id || kpiData.initiativeId);
      setUnit(kpiData.unit);
      setPeriodicity(kpiData.periodicity);
      setResponsible(kpiData.responsible._id || kpiData.responsible);
      setFormula(kpiData.formula);
      setTarget(kpiData.target.toString());
      setMinTolerance(kpiData.tolerance.minimum.toString());
      setWarningThreshold(kpiData.tolerance.warningThreshold.toString());
      setKpiType(kpiData.kpiType);
      setTags(kpiData.tags || []);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Verificar si hay cambios que requieren versionado
    const requiresVersioning =
      formula !== kpi?.formula ||
      targetNum !== kpi?.target ||
      minToleranceNum !== kpi?.tolerance.minimum ||
      warningThresholdNum !== kpi?.tolerance.warningThreshold;

    if (requiresVersioning && !versionChanges.trim()) {
      alert('Por favor describe los cambios realizados para el versionado');
      return;
    }
    ...
    try {
      const res = await fetch(`/api/kpis/${kpiId}`, {
        ...
        body: JSON.stringify({
          ...
          kpiType,
          tags,
          versionChanges: requiresVersioning ? versionChanges : undefined,
        }),
      });
      ...
      if (res.ok) {
        ...
        alert(error.error || 'Error al actualizar KPI');
      }
    } catch (error) {
      console.error('Error updating KPI:', error);
      alert('Error al actualizar KPI');
    } finally {
      setSaving(false);
    }
  };
  ...
  const requiresVersioning =
    formula !== kpi?.formula ||
    parseFloat(target) !== kpi?.target ||
    parseFloat(minTolerance) !== kpi?.tolerance.minimum ||
    parseFloat(warningThreshold) !== kpi?.tolerance.warningThreshold;
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="mb-6">
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            Modifica el indicador clave de rendimiento
          </p>
          {kpi && (
            <div className="mt-2 flex items-center space-x-4 text-sm">
              <span className="text-gray-600 dark:text-gray-400">
                Estado: <span className="font-semibold">{kpi.status}</span>
              </span>
              <span className="text-gray-600 dark:text-gray-400">
                Versión: <span className="font-semibold">v{kpi.currentVersion}</span>
              </span>
            </div>
          )}
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 space-y-6">
          ...
          <div className="border-b border-gray-200 dark:border-gray-700 pb-6">
            ...
            <IframeFormulaEditor value={formula} onChange={setFormula} />
          </div>
          ...
          {/* Versionado - mostrar solo si hay cambios importantes */}
          {requiresVersioning && (
            <div className="border-b border-gray-200 dark:border-gray-700 pb-6">
              <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4 mb-4">
                <h3 className="text-sm font-semibold text-yellow-900 dark:text-yellow-300 mb-2">
                  ⚠️ Cambios que requieren versionado
                </h3>
                <p className="text-sm text-yellow-800 dark:text-yellow-200">
                  Has modificado la fórmula, meta o tolerancia. Esto creará una nueva versión del KPI.
                  Por favor describe los cambios realizados.
                </p>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                  rows={3}
                  placeholder="Ej: Ajuste de fórmula para mejor precisión, actualización de meta trimestral"
                  required
                />
              </div>
            </div>
